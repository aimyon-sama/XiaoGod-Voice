...
def main() -> None:
    ...
    if speaker not in speakers:
        raise ValueError(f"Unknown speaker '{speaker}'. Available: {', '.join(speakers)}")
    ...
    speaker_id = torch.tensor([speakers[speaker]], dtype=torch.long, device=args.device)
    with torch.no_grad():
    ...
    print(json.dumps({"out": args.out, "speaker": speaker, "sample_rate": audio_cfg.sample_rate}, ensure_ascii=False))
