...
def main() -> None:
    ...
    tmp_model = XiaoGodTTS(old_model_cfg, audio_cfg.n_mels, vocab.pad_id).to(args.device)
    tmp_model.load_state_dict(ckpt["model"])
    expand_speaker_embedding(tmp_model, len(speakers))
